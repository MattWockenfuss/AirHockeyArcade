...
int titleWindow(sf::RenderWindow &window) {
	...
	*/
	...
	sf::Text title_text(title_font, "Air Hockey Arcade", 20*screenRatio);
	const sf::FloatRect titleRect = title_text.getLocalBounds();
	title_text.setOrigin(titleRect.getCenter());
	title_text.setPosition(sf::Vector2f(window.getSize().x / 2.0f, 36*screenRatio));

	//Insert coin to be displayed at bottom of screen. Flashes.
	sf::Text play_text(give_money_font, "Press Enter To Change Screen!", 8*screenRatio);
	const sf::FloatRect textRect = play_text.getLocalBounds();
	play_text.setOrigin(textRect.getCenter());
	play_text.setPosition(sf::Vector2f(window.getSize().x / 2.0f, 150*screenRatio));
...
int gameSelectWindow(sf::RenderWindow &window){ // arrows don't always animate, not sure why
	...
	sf::Texture arrows[4];
	bool loadImage;
	loadImage = arrows[0].loadFromFile("assets/images/gameSelectArrow_L1.png");
	if(!loadImage) {std::perror("File not found!"); }
	loadImage = arrows[1].loadFromFile("assets/images/gameSelectArrow_L2.png");
	if(!loadImage) {std::perror("File not found!"); }
	loadImage = arrows[2].loadFromFile("assets/images/gameSelectArrow_R1.png");
	if(!loadImage) {std::perror("File not found!"); }
...
int loadingWindow(sf::RenderWindow &window, sf::String name){
	...
	while ( window.isOpen() ) {
		...
		while (std::optional event = window.pollEvent()) {
			...
			if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Enter) && enterUnPressed && !nameLoaded){
				comName = "";
				comName += (char)( (rand()%26) + 'A');
				comName += (char)( (rand()%26) + 'A');
				comName += (char)( (rand()%26) + 'A');
				nameLoaded = true;
				enterUnPressed = false;
			}
			if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Enter) && enterUnPressed && nameLoaded){ // go to next screen
				return 1;
			}
		}
		...
		for(int i = 0; i<3; i++){
			...
			if(frames[i]>=2.5){
				frames[i] = 0;
				if(nameLoaded && nameLoads[i]==0){ // set ready to load letter of name
					nameLoads[i] = 1;
				}
			}
			if(nameLoads[i]==1 && nameLoads[0]>=1 && frames[i]>=0.6 && frames[i]-dt<=0.6){ // display letter of name
				nameLoads[i] = 2;
				sf::String comTextName = com_text.getString();
				comTextName += comName[i];
				com_text.setString(comTextName);
			}
		}
