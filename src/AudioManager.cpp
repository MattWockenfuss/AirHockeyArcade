...
void AudioManager::playSound(sf::Music &sound) {
    sound.setLooping(false);
    sound.play();
    muslist.push_back(&sound);
    checkSound();
}
...
void AudioManager::checkSound() {
    std::list<sf::Music*> tbr = {};
    // std::cout << "Size of muslist: " << muslist.size() << std::endl;
    // size_t sizet = muslist.size();
    for (const auto &sun : muslist) {
        // std::cout << "Size of muslist: " << muslist.size() << std::endl;
        // std::cout << "Sound address: " << &sun << std::endl;
        // if (sun == nullptr) std::cout << "Sound is nullptr!!! SEGFAULT" << std::endl;
        // std::cout << "Sound status: " << static_cast<int>(sun->getStatus()) << std::endl;
        if (sun->getStatus() != sf::SoundSource::Status::Playing) {
            tbr.push_front(sun);
        }
        // else std::cout << "Sound: " << &sun << " is playing!" << std::endl;
    }
    while (tbr.size() != 0) {
        muslist.remove(tbr.back());
        tbr.pop_back();
    }
    // std::cout << "New size of muslist: " << muslist.size() << std::endl;
}

void AudioManager::stopSound() {
    if (muslist.size() == 0) return;
    for (const auto &sun : muslist) {
        sun->stop();
    }
    muslist.clear();
    // std::cout << "muslist cleared!" << std::endl;
    // std::cout << "Size of muslist: " << muslist.size() << std::endl;
}