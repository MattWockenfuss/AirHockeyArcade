...
void NameEntryState::init(Context* ctx){
    ...
    */
    ...
    charLabel.emplace(ctx -> assets -> getFont("ArcadeNormal"), "", 22);
...
void NameEntryState::p1render(sf::RenderTexture& p1window) {
    ...
    if(!p1ready){
        ...
        if(caretVisible){
            borderSquare.setSize({6.0f, squareWidth + gap + (bw * 2)});
            //borderSquare.setOrigin(charLabel -> getLocalBounds().getCenter()); //where we set its position is the center of the string/char
            borderSquare.setPosition({startX + (46.0f * p1name.length()), startY - ((gap * 3) + squareWidth + (bw * 2) + 4.0f)});
...
void NameEntryState::p2render(sf::RenderTexture& p2window) {
    ...
    if(!p2ready){
        ...
        if(caretVisible){
            borderSquare.setSize({6.0f, squareWidth + gap + (bw * 2)});
            //borderSquare.setOrigin(charLabel -> getLocalBounds().getCenter()); //where we set its position is the center of the string/char
            borderSquare.setPosition({startX + (46.0f * p2name.length()), startY - ((gap * 3) + squareWidth + (bw * 2) + 4.0f)});
